...
def load_vgg(sess, vgg_path):
    ...
    #sess.graph.get_operations(), this returns a list of all layers, then query each layer to get shape info
    #For example: tf.get_default_graph().get_tensor_by_name('image_input:0')
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    # upsample. The stride of 2x2 is what upsamples the image by factor of 2
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    # The nn_last_layer returns [batch #, height, width,class] which is 4D. This gets converted/flattened to pixel and clss 2D matrix.
    ...
    for i in range(epochs):
        ...
        for image, label in get_batches_fn(batch_size):
            _, loss = sess.run([train_op, cross_entropy_loss], 
                               feed_dict={input_image: image, correct_label: label,keep_prob: 0.6, learning_rate: 0.0009})
...
def run():
    ...
    with tf.Session() as sess:
        ...
        final_layer_out = layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes)

        logits, train_op, cross_entropy_loss = optimize(final_layer_out, correct_label, learning_rate, num_classes)
